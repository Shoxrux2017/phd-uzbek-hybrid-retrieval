...
H2a и H2b проверяются и интерпретируются раздельно: изменение взаимодополняемости само по себе не подтверждает изменение эффекта гибридного поиска. Обе части требуют практической значимости и воспроизводимости соответствующего изменения, как указано в H2; H2a сопоставляется с H0₂a, H2b — с H0₂b. Превосходство `hybrid > best standalone` заранее не предполагается.
...
**H0₂a:** При фиксированной `D` переход между `raw`, `stem` и `lemma` не вызывает практически значимых и воспроизводимых изменений set-based lexical–dense complementarity, включая:
...
**H0₂b:** При фиксированных `D` и протоколе объединения различия incremental hybrid gain между `raw`, `stem` и `lemma` находятся в заранее определённых пределах практически незначимого эффекта либо не являются воспроизводимыми.
...
#### Interpretation matrix — H2a / H2b

| Эмпирический результат | Научная интерпретация |
|---|---|
| **H2a supported + H2b supported** | Свидетельства поддерживают полную рабочую цепочку `morphology → complementarity change → hybrid-gain change` для соответствующего перехода между морфологическими вариантами. |
| **H2a supported + H0₂b supported** | Морфологическое представление меняет взаимодополняемость, но свидетельства не показывают практически значимого и воспроизводимого переноса этого изменения в incremental hybrid gain; часть текущей постановки, относящаяся к hybrid gain, должна быть пересмотрена или сужена. |
| **H0₂a convincingly supported** | Центральное основание **`v0.8 refined`** ослабляется, и gap должен быть пересмотрен. |
| **Insufficient evidence** | Недостаточность свидетельств по любой из частей нельзя интерпретировать как поддержку соответствующей H0. |

Поддержка H0₂a / H0₂b требует убедительных эмпирических свидетельств: недостаток данных о воспроизводимости не доказывает невоспроизводимость. Отрицательный результат является допустимым научным результатом.

**`failure to reject H0 != proof of no effect`**: неотклонение нулевой гипотезы не доказывает отсутствие эффекта. `BENCHMARK_QRELS_PROTOCOL_v0.1` различает:
...
В уже созданном [BENCHMARK_QRELS_PROTOCOL_v0.1](BENCHMARK_QRELS_PROTOCOL_v0.1.md), имеющем статус **WORKING / PROVISIONAL**, определена статистическая схема: парный анализ на уровне запросов; парные рандомизационные/пермутационные тесты для знаковых различий эффективности; поправка Holm для морфологических контрастов, включая отдельные семейства пороговых тестов H1 и H2a; логика эквивалентности / практической незначимости с TOST или доверительными интервалами там, где это применимо; бутстрэп-оценка неопределённости; BH-FDR для RQ3; требование проспективного анализа мощности/чувствительности. Окончательные научно обоснованные границы практической эквивалентности (equivalence margins), минимальный практически значимый размер эффекта (SESOI), некоторые точные реализации статистических тестов и ресэмплинга, итоговый размер выборки и объём подтверждающего анализа ещё не зафиксированы. Их необходимо обосновать и зафиксировать в предтестовой редакции протокола и `EXPERIMENT_LOCK` до доступа исследователя и участников настройки к тестовым меткам релевантности (test-label exposure); приведённые в v0.1 численные пороги остаются плановыми кандидатами. Здесь новые пороги и тесты не вводятся.
...
#### RQ1/H1 — pairwise lexical relevant-set comparison

Для каждого запроса `q` множество `R_m(q,k)` содержит релевантные документы среди `top-k` результатов `BM25_m`, где `m ∈ {raw, stem, lemma}`; релевантность определяется по общим qrels.

Для непосредственной проверки изменения **состава** найденных BM25 релевантных документов выполняются попарные сравнения `raw ↔ stem`, `raw ↔ lemma` и `stem ↔ lemma` при одинаковой глубине `k`. Для каждой пары анализируются:

- релевантные документы, уникальные для первого варианта (relevant hits unique to the first variant);
- релевантные документы, уникальные для второго варианта (relevant hits unique to the second variant);
- пересечение множеств (intersection);
- степень перекрытия (overlap);
- при необходимости симметрическая разность (symmetric difference).

Конкретная формула нормированного overlap/Jaccard и численные пороги будут определены в `BENCHMARK_QRELS_PROTOCOL_v0.1`; здесь они не фиксируются.

Совокупная эффективность BM25 (aggregate BM25 effectiveness) и состав найденных релевантных документов (relevant-set composition) — разные оцениваемые результаты. Одинаковая или близкая совокупная эффективность не означает одинаковый состав найденных релевантных документов.

#### RQ2/H2 — lexical–dense complementarity and hybrid gain
